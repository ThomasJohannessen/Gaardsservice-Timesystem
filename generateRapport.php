...
<head>
    ...
    <title>Generere Arbeidsrapport</title>
...
<body>
    <?php
        $fraDato = date('Y-m-d', strtotime(date('Y-m-d'). ' - 13 days'));
        $tilDato = date('Y-m-d', strtotime(date('Y-m-d')));

        $secondweek = date("W"); 
        $firstweek = $secondweek - 1;
        $title = "<h2> Timeliste for Thomas J uke: $firstweek og $secondweek </h2>";

        echo $title;
    ?>

    <button onclick="window.print();return false;">Generer rapport!</button>
